...
\section{Optuna Optimization}

\subsection{Optuna's performances}

\begin{wrapfigure}{r}{0.5\textwidth}
    \centering
    \includegraphics[width=\linewidth]{../img/OPTUNA_1/optuna_trial_duration.png}
    \caption{Duration of individual Optuna trials}
\end{wrapfigure}

\indent This week, we conducted a hyperparameter optimization of our model using Optuna. The study focused on tuning four key parameters : the number of epochs, the number of episodes, the batch size and the learning rate. Although the experiment was initially planned for 100 trials, the optimization process ran for approximately four days and completed only 59 trials in total. Due to the large episode and epoch sizes required, each trial took on average about three hours to train, making the overall process computationally expensive and time-consuming.

\noindent The optimization objective was to maximize the average trade value on the test data to fine-tune the model's performance. As shown below, most trials achieved positive results, with two noticeable outliers reaching values of 0.24 and 0.41. Although these scores appeared promising, further analysis revealed that this target metric was not well-suited for improving the model's generalization and stability.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{../img/OPTUNA_1/optuna_trial_goals.png}
    \caption{Overview of Optuna trial performance metrics}
\end{figure}
...
\begin{wrapfigure}{l}{0.5\linewidth}
    ...
    \caption{Relative importance of hyperparameters according to Optuna analysis}
    \vspace{-1cm}
\end{wrapfigure}

\noindent Finally, the following plot illustrates the relative importance of each hyperparameter. Consistent with previous reports, the number of epochs and number of episodes had the greatest influence on the model's overall performance, confirming their critical role in training dynamics and model convergence.\par
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{../img/OPTUNA_1/trial_001_train.png}
    \caption{Trial 1 - Training results}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{../img/OPTUNA_1/trial_001_test.png}
    \caption{Trial 1 - Testing results}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{../img/OPTUNA_1/trial_014_train.png}
    \caption{Trial 14 - Training results}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{../img/OPTUNA_1/trial_014_test.png}
    \caption{Trial 14 - Testing results}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{../img/OPTUNA_1/trial_022_train.png}
    \caption{Trial 22 - Training results}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{../img/OPTUNA_1/trial_022_test.png}
    \caption{Trial 22 - Testing results}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{../img/OPTUNA_1/trial_033_train.png}
    \caption{Trial 33 - Training results}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{../img/OPTUNA_1/trial_033_test.png}
    \caption{Trial 33 - Testing results}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{../img/OPTUNA_1/trial_051_train.png}
    \caption{Trial 51 - Training results}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{../img/OPTUNA_1/trial_051_test.png}
    \caption{Trial 51 - Testing results}
\end{figure}
...
\section{Optuna Conslusion}

\subsection{Optuna's results Overview}
...
\begin{figure}[H]
    ...
    \caption{Overview of each trial performances}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{tabular}{|c|c|c|c|c|c|}
        \hline
        \textbf{Trial} & \textbf{Episode} & \textbf{Epochs} & \textbf{Batsh size} & \textbf{Learning rate} & \textbf{Average profit} ($\$$/year)\\ \hline
        \textit{01} & $136$ & $23$ & $128$ & $0.36$ & $10$k \\
        \textit{14} & $194$ & $72$ & $128$ & $1.00$ & $16$k \\
        \textit{22} & $199$ & $58$ & $128$ & $0.96$ & $20$k \\
        \textit{33} & $127$ & $33$ &  $32$ & $0.32$ & $17$k \\
        \textit{51} & $195$ & $25$ &  $64$ & $0.52$ & $11$k \\
        \hline
    \end{tabular}
    }
    \caption{Overview of the trial parameters}
\end{figure}

\noindent The results indicate that the reward parameter does not significantly influence the quality of fine-tuning on the test dataset. However, it also appears that the current Optuna optimization objective is not well aligned with our model's behavior. Therefore, it would be beneficial to define a new objective function for instance, using the sharpe ratio as an optimization target could provide a more reliable measure of performance. Additionally, some hyperparameters, such as the batch size and learning rate, seem to have stable optimal values (approximately 128 and 0.3, respectively) and could be fixed in future optimization runs to reduce search complexity.


\subsection{Fine tuning the model}

\indent To achieve better performance, several modifications and experiments are planned for the next round of Optuna optimization. The main improvements include:

\begin{itemize}
    \item \textbf{Optimization Objective :} Redefine Optuna's objective function to use the \textbf{Sharpe ratio}, allowing the optimization process to balance both drawdown and profit simultaneously
    
    \item \textbf{Tick Interval Adjustment :} As mentioned in previous reports, the tick interval should be modified to a \textbf{5-minute timeframe} to evaluate the model's responsiveness and stability at a finer temporal resolution
    
    \item \textbf{Additional Input Features :} Following prior recommendations, we plan to include new input variables such as the \textbf{VVIX} and the \textbf{S\&P 500 index}, which could enhance the model's ability to capture broader market volatility and sentiment
\end{itemize}
...
\indent The previous environment was modified to create a new one designed to operate at a higher resolution and incorporate more dynamic behaviors. The main improvements are :

\begin{enumerate}
    \item Inclusion of a \textbf{5-minute tick} in the state space
    \item Addition of \textbf{short positions}
    \item Optuna optimization using \textbf{maximum drawdown} and \textbf{average profit} as objectives
\end{enumerate}

\noindent However, after testing the new model, several issues surfaced. Due to the increased number of ticks, RAM usage grew significantly. The first long-run experiment, executed over a weekend, resulted in a complete system crash: the operating system ran out of memory and stopped all processes, preventing the training from resuming automatically. After debugging, we confirmed that RAM saturation was the cause.

\noindent Secondly, the sequence length had to be reduced from one week to a single day because of GPU memory limitations. When the sequence exceeds 288 samples and the batch size is set to 256 (73,728 items per batch), the VRAM cannot handle the workload.

\vfill

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{../img/new_env_overview.png}
...
\indent We conducted a 72-hour Optuna search to identify promising hyperparameters. Below are the results on both the training and testing datasets. The model was trained using the following parameters :
...
\noindent Although the number of epochs and episodes is relatively low, the training duration was long---almost three days---due to the environment's complexity and computational requirements.
...
\noindent The training curve shows that the agent dipped into negative profitability multiple times. Although the final cumulative reward is positive, the average profit remains negative, indicating unstable or inconsistent behavior.
...
\indent As anticipated, the model performs poorly on the testing dataset. The volatility is extremely high, the Sharpe ratio is negative, and the average profit is significantly worse than during training. Additional training cycles, preferably during the week to avoid long unattended runtime failures, are necessary to stabilize and improve the model.

\newpage

\section{Optuna best results}

\indent We ran an additional week of Optuna trials, expecting improved performance during testing. Over this week, two processes were running in parallel, and the scripts trained a total of ten agents using various batch sizes (32, 64, 128, 256), with episodes ranging between 50 and 100 and epochs also ranging between 50 and 100. Among all configurations, Trial 10 produced the best results during the evaluation phase.
 

\subsection{Training - Trial 10}

\indent The training phase exhibits weak performance, likely influenced by a pronounced market decline that occurred during this period. Despite this, the agent demonstrates substantially better results when evaluated on the testing dataset.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{../img/OPTUNA_2/trial10_train.png}
    \caption{Agent performance on the training dataset}
\end{figure}

\newpage

\subsection{Testing - Trial 10}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{../img/OPTUNA_2/trial10_test.png}
    \caption{Agent performance on the testing dataset}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{tabular}{|l|l|}
        \hline
        \textbf{Model Testing Result (threshold=0.65)} & \\ \hline
        \textbf{total\_reward}      & 6{,}431{,}756     \\
        \textbf{action\_per\_day}   & 6                 \\
        \textbf{annual\_return}     & 0.113             \\
        \textbf{average\_profit}    & -0.034\%          \\
        \textbf{annual\_volatility} & 54.216            \\
        \textbf{sharpe\_ratio}      & -0.203            \\
        \textbf{max\_drawdown}      & -1.398            \\
        \hline
    \end{tabular}
\end{figure}

\indent We can observe a relatively strong equity curve, suggesting that the agent achieved good generalization of the learned policy. However, further evaluation is necessary. Additional testing on a larger and more diverse set of datasets is required to gather sufficient statistics and properly assess the robustness of the model.
